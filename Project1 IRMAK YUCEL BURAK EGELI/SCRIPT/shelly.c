...
char  *value[MAX_LINE];
  ...
  while (shouldrun){            		/* Program terminates normally inside setup */
    ...
    if(strncmp(inputBuffer,"script",4) == 0)//Part 2 Script Command, does not work with bookmark or I/O for I/O comment all 							additional commands
    ...
    /*if(strncmp(inputBuffer,"bookmark",4) == 0)

	fbookmark=fopen("bookmarks.txt","a+");
	...
	while(strncmp (inputBuffer,key[0],4) == 0){
	
}
	
	  
 
   	if(strncmp(inputBuffer,"wforecast",4) == 0)

    	if(strncmp(inputBuffer,"alarm",4) == 0)
	falarm=fopen("dailyalarm.txt","a+");
	fputs("\n" ,falarm);
	fputs("wake up" ,falarm); */
...
//1st Part of the Assignment
	...
	//exit(1);   //uncomment while bookmark
